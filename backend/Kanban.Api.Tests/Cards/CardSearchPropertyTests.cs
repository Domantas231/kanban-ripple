...
public sealed class CardSearchPropertyTests
{
    ...
    public async Task Property_52_SearchCompleteness_SubstringCaseInsensitiveScopedAndExcludesArchived()
    {
        ...
        var projectA = await fixture.ProjectService.CreateAsync(ownerId, "Search Project A");
        ...
        var projectB = await fixture.ProjectService.CreateAsync(ownerId, "Search Project B");
        ...
        var matchByTitle = await fixture.CardService.CreateAsync(
            columnA.Id,
            ownerId,
            new CreateCardDto("Kanban implementation", "non-matching description", null, null, null));

        var matchByDescription = await fixture.CardService.CreateAsync(
            columnA.Id,
            ownerId,
            new CreateCardDto("Search docs", "Design notes for KANBAN search", null, null, null));

        var nonMatchInScope = await fixture.CardService.CreateAsync(
            columnA.Id,
            ownerId,
            new CreateCardDto("Sprint planning", "no keyword here", null, null, null));

        var outOfScopeMatch = await fixture.CardService.CreateAsync(
            columnB.Id,
            ownerId,
            new CreateCardDto("Kanban in other project", "should not be returned", null, null, null));

        var archivedMatch = await fixture.CardService.CreateAsync(
            columnA.Id,
            ownerId,
            new CreateCardDto("Archived Kanban Card", "should be filtered", null, null, null));
    ...
    public async Task Property_53_SearchResultsIncludeTitleDescriptionPreviewAndLocationMetadata()
    {
        ...
        var project = await fixture.ProjectService.CreateAsync(ownerId, "Search Metadata Project");
    ...
    private sealed class PostgresSearchFixture : IAsyncDisposable
    {
        ...
        {
            // Reuses the shared Testcontainers Postgres so this test does not require
            // docker-compose to be running locally and no hard-coded credentials leak.
            var database = await PostgresTestContainer.CreateDatabaseAsync();
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            // The Testcontainers Ryuk reaper will remove the entire Postgres container
            // when the test process exits — no per-fixture cleanup needed.
        }
    }
}