...
public sealed class SubscriptionNotificationPropertyTests
{
    ...
    public async Task Property_69_ModifyingSubscribedCardNotifiesAllSubscribersExceptModifier()
    {
        ...
        await fixture.CardService.UpdateAsync(
            card.Id,
            actorId,
            new UpdateCardDto("Updated title", "Updated description", card.StartDate, card.DueDate, null, card.Version));

        // CardService walks the entity tree (Card -> Column -> Board -> Project), querying subscribers at each scope.
        Assert.Equal(4, fixture.SubscriptionService.GetSubscriberCalls.Count);
        var queriedScopes = fixture.SubscriptionService.GetSubscriberCalls.Select(x => x.EntityType).ToHashSet();
        Assert.Contains(EntityType.Card, queriedScopes);
        Assert.Contains(EntityType.Column, queriedScopes);
        Assert.Contains(EntityType.Board, queriedScopes);
        Assert.Contains(EntityType.Project, queriedScopes);
    ...
    public async Task Property_70_NotificationIncludesChangeTypeAndModifierIdentity()
    {
        ...
        var updated = await fixture.CardService.UpdateAsync(
            card.Id,
            actorId,
            new UpdateCardDto("Renamed for property 70", "Body", card.StartDate, card.DueDate, null, card.Version));

        var notification = Assert.Single(fixture.NotificationService.CreateCalls);

        Assert.Equal(subscriberId, notification.UserId);
        Assert.Equal(NotificationType.Updated, notification.Type);
        Assert.Contains("Task updated", notification.Title);
    ...
    private static async Task<TestFixture> CreateFixtureAsync()
    {
        ...
        var cardService = TestServiceBuilder.BuildCardService(dbContext, notificationService, subscriptionService);
    ...
    private sealed class RecordingSubscriptionService : ISubscriptionService
    {
        ...
        public Task<IReadOnlyList<Kanban.Api.Services.Subscriptions.MySubscriptionDto>> GetMySubscriptionsAsync(Guid userId)
        {
            throw new NotSupportedException();
        }
    }
