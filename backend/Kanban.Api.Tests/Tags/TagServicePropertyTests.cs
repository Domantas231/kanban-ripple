...
public class TagServicePropertyTests
{
    ...
    public async Task Property_59_CreateWithoutNameOrColorReturnsError()
    {
        ...
        var project = await fixture.ProjectService.CreateAsync(ownerId, "Tag create validation project");
        var board = await fixture.BoardService.CreateAsync(project.Id, ownerId, "Main board");

        await Assert.ThrowsAsync<BadRequestException>(
            () => fixture.TagService.CreateAsync(board.Id, ownerId, new CreateTagDto("", "#AABBCC")));

        await Assert.ThrowsAsync<BadRequestException>(
            () => fixture.TagService.CreateAsync(board.Id, ownerId, new CreateTagDto("   ", "#AABBCC")));

        await Assert.ThrowsAsync<BadRequestException>(
            () => fixture.TagService.CreateAsync(board.Id, ownerId, new CreateTagDto("bug", "")));
    ...
    public async Task Property_60_UpdateNameAndColorIsReflectedOnAllCardsUsingTag()
    {
        var fixture = CreateFixture();
        var ownerId = Guid.NewGuid();
        var project = await fixture.ProjectService.CreateAsync(ownerId, "Tag update propagation project");
        var board = await fixture.BoardService.CreateAsync(project.Id, ownerId, "Main board");
        var column = await fixture.ColumnService.CreateAsync(board.Id, ownerId, "Todo");

        var firstCard = await fixture.CardService.CreateAsync(column.Id, ownerId, new CreateCardDto("Card 1", null, null, null, null));
        var secondCard = await fixture.CardService.CreateAsync(column.Id, ownerId, new CreateCardDto("Card 2", null, null, null, null));

        var tag = await fixture.TagService.CreateAsync(board.Id, ownerId, new CreateTagDto("Bug", "#112233"));
        ...
        var listedTags = await fixture.TagService.ListAsync(board.Id, ownerId);
    ...
    public async Task Property_61_DeleteTagRemovesAssociationFromAllCards()
    {
        var fixture = CreateFixture();
        var ownerId = Guid.NewGuid();
        var project = await fixture.ProjectService.CreateAsync(ownerId, "Tag deletion project");
        var board = await fixture.BoardService.CreateAsync(project.Id, ownerId, "Main board");
        var column = await fixture.ColumnService.CreateAsync(board.Id, ownerId, "Todo");

        var firstCard = await fixture.CardService.CreateAsync(column.Id, ownerId, new CreateCardDto("Card 1", null, null, null, null));
        var secondCard = await fixture.CardService.CreateAsync(column.Id, ownerId, new CreateCardDto("Card 2", null, null, null, null));
        var tag = await fixture.TagService.CreateAsync(board.Id, ownerId, new CreateTagDto("Ops", "#445566"));
    ...
    public async Task Property_62_ColorIsStoredAndReturnedCorrectlyInQueries()
    {
        var fixture = CreateFixture();
        var ownerId = Guid.NewGuid();
        var project = await fixture.ProjectService.CreateAsync(ownerId, "Tag color property project");
        var board = await fixture.BoardService.CreateAsync(project.Id, ownerId, "Main board");
        var column = await fixture.ColumnService.CreateAsync(board.Id, ownerId, "Todo");
        var card = await fixture.CardService.CreateAsync(column.Id, ownerId, new CreateCardDto("Card", null, null, null, null));

        var created = await fixture.TagService.CreateAsync(board.Id, ownerId, new CreateTagDto("Design", "#a1b2c3"));
        ...
        var listed = await fixture.TagService.ListAsync(board.Id, ownerId);
    ...
    private static TestFixture CreateFixture()
    {
        ...
        var tagService = TestServiceBuilder.BuildTagService(dbContext);
