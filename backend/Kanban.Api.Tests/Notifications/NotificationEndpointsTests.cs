...
public sealed class NotificationEndpointsTests : IClassFixture<ProjectsApiFactory>
{
    ...
    public async Task Assignment_CreatesNotificationForAssignee()
    {
        ...
        Assert.Equal(NotificationType.Assigned, created.Type);
    ...
    public async Task MarkAsRead_ReturnsOk()
    {
        ...
        var notification = await CreateNotificationAsync(userId, NotificationType.Updated, "Needs reading", "Please read");
    ...
    public async Task MarkAllAsRead_UpdatesAllUnread()
    {
        ...
        var firstUnread = await CreateNotificationAsync(userId, NotificationType.Created, "U1", "U1");
        _ = await CreateNotificationAsync(userId, NotificationType.Updated, "U2", "U2");
        var alreadyRead = await CreateNotificationAsync(userId, NotificationType.Moved, "Read", "Read");
        var otherUnread = await CreateNotificationAsync(otherUserId, NotificationType.Deleted, "Other", "Other");
    ...
    public async Task Delete_ReturnsNoContent()
    {
        ...
        var notification = await CreateNotificationAsync(userId, NotificationType.Created, "Delete me", "Delete me");
    ...
    public async Task AccessingOtherUsersNotification_ReturnsNotFound()
    {
        ...
        var otherNotification = await CreateNotificationAsync(otherUserId, NotificationType.Updated, "Other", "Other");
