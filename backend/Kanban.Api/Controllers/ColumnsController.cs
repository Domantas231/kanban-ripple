...
public sealed class ColumnsController : ControllerBase
{
    ...
    public async Task<ActionResult<Column>> Create(Guid boardId, [FromBody] CreateColumnRequest request)
    {
        if (!TryGetUserId(out var userId))
        {
            return Unauthorized(new { message = "Invalid authenticated user." });
        ...
        try
        {
            var result = await _columnService.CreateAsync(boardId, userId, request.Name);
            return Ok(result);
        }
        catch (KeyNotFoundException ex)
        {
    ...
    public async Task<ActionResult<Column>> Update(Guid id, [FromBody] UpdateColumnRequest request)
    {
        if (!TryGetUserId(out var userId))
        {
            return Unauthorized(new { message = "Invalid authenticated user." });
        ...
        try
        {
            var result = await _columnService.UpdateAsync(id, userId, new UpdateColumnDto(request.Name));
            return Ok(result);
        }
        catch (KeyNotFoundException ex)
        {
    ...
    public async Task<IActionResult> Archive(Guid id)
    {
        ...
        try
        {
            await _columnService.ArchiveAsync(id, userId);
            return NoContent();
        }
        catch (KeyNotFoundException ex)
        {
    ...
    public async Task<IActionResult> Restore(Guid id)
    {
        ...
        try
        {
            await _columnService.RestoreAsync(id, userId);
            return NoContent();
        }
        catch (KeyNotFoundException ex)
        {
    ...
    public async Task<ActionResult<Column>> Reorder(Guid id, [FromBody] ReorderColumnRequest request)
    {
        if (!TryGetUserId(out var userId))
        {
            return Unauthorized(new { message = "Invalid authenticated user." });
